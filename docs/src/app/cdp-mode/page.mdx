...
On Tizen, `open <url>` in local-launch mode passes the initial URL directly to `ubrowser`. That avoids relying on `Target.createTarget` for the first page when the browser-level inspector socket is up before a page target becomes attachable. Later commands still require an attachable page target from the Tizen CDP backend.
